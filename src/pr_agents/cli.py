...
def analyze_pr(args):
    ...
    coordinator = PRCoordinator(
        ...
        ai_enabled=args.ai_provider is not None
    )
...
def analyze_unreleased(args):
    ...
    if args.output:
        ...
        if pr_count > 0:
            print(f"\nSummary:")
            print(f"- Successful analyses: {batch_summary.get('successful_analyses', 0)}")
...
Examples:
  ...
  pr-agents batch --repo owner/repo --count 5 --ai google-adk
    ...
    # Analyze unreleased PRs
    unreleased_parser = subparsers.add_parser("unreleased", help="Analyze PRs merged but not yet released")
    unreleased_parser.add_argument("repo", help="Repository (owner/name)")
    unreleased_parser.add_argument(
        "--branch", 
        default="main", 
        help="Base branch to check (default: main)"
    )
    ...
    unreleased_parser.add_argument("--ai", dest="ai_provider", help="Enable AI summaries")
    unreleased_parser.add_argument(
        "--personas",
        help="Comma-separated list of personas to include (default: all)",
        default=None
    ...
    batch_parser.add_argument("--count", type=int, help="Number of recent PRs to analyze")
