...
class SummaryValidator:
    ...
    def __init__(self):
        ...
        self.quality_patterns = {
            ...
            "generic": re.compile(
                r"^(This PR|This commit|These changes|The code)", re.IGNORECASE
            ),
        }

    def validate_summary(
        self, summary: str, persona: str, code_context: dict = None
    ) -> tuple[bool, list[str]]:
        ...
        if word_count < min_words:
            issues.append(
                f"Summary too short: {word_count} words (minimum: {min_words})"
            )
        elif word_count > max_words:
            issues.append(
                f"Summary too long: {word_count} words (maximum: {max_words})"
            )
        ...
        if code_context:
            validation_issues = self._validate_against_context(
                summary, code_context, persona
            )
        ...
        if not is_valid:
            logger.warning(
                f"Summary validation failed for {persona}: {', '.join(issues)}"
            )
    ...
    def _validate_against_context(
        self, summary: str, code_context: dict, persona: str
    ) -> list[str]:
        ...
        if file_mentions and "file_diffs" in code_context:
            actual_files = {
                diff.get("filename", "") for diff in code_context["file_diffs"]
            }
...
class SummaryEnhancer:
    ...
    def _make_specific(self, summary: str, context: dict) -> str:
        ...
        generic_starters = ["This PR ", "This commit ", "These changes ", "The code "]
