...
class ClaudeDirectProvider(BaseLLMProvider):
        ...
        for line in lines:
            ...
            if "Repository:" in line and "(" in line:
                ...
                if "(" in repo_parts:
                    ...
                    info["repo_type"] = (
                        repo_parts.split("(")[1].replace(")", "").strip()
                    )
    ...
    def _generate_executive_summary(self, pr_info: dict) -> str:
        ...
        util_files = [
            ...
            for f in files
            if any(
                pattern in f.lower() for pattern in ["util", "helper", "lib", "core"]
            )
            and "test" not in f.lower()
        ]
        config_files = [
            f
            for f in files
            if any(ext in f.lower() for ext in [".json", ".yml", ".yaml", "config"])
        ]
    ...
    def _generate_product_summary(self, pr_info: dict) -> str:
        ...
        if repo_type == "prebid" and bid_adapters:
            ...
            test_coverage = (
                f" Includes {len(test_files)} test file{'s' if len(test_files) != 1 else ''} validating functionality."
                if has_tests
                else ""
            )
            ...
            if additions > 200 and deletions < 50:
                media_types = []
                if has_video or additions > 300:
                    media_types.append("video")
                if has_native or additions > 250:
                    media_types.append("native")
                media_types.append("banner")  # Always support banner

                media_str = (
                    " and ".join(media_types)
            ...
            # Feature enhancement (balanced changes)
                ...
                features = []
                if has_config:
                    features.append("configuration parameters")
                if has_sync:
                    features.append("user synchronization")
                if has_video:
                    features.append("video support")
                if has_native:
                    features.append("native ad formats")

                if features:
                    feature_str = (
                        " and ".join(features) if len(features) > 1 else features[0]
                    )
        ...
        # Non-adapter changes
            ...
            component_type = (
                "platform utilities"
                if any("util" in f.lower() for f in files)
                else "system components"
            )
    ...
    def _generate_developer_summary(self, pr_info: dict) -> str:
        ...
        if repo_type == "prebid" and bid_adapters:
            ...
            if additions > 200 and deletions < 50:
                test_info = (
                    f"Test suite includes {len(test_files)} spec files with unit tests covering bid request building, response parsing, and error handling."
                    if test_files
                    else "No test coverage included."
                )
...
{files_str}"""
            ...
            # Feature additions
                ...
                test_desc = (
                    f" Test coverage updated across {len(test_files)} files."
                    if test_files
                    else ""
                )
...
{files_str}"""
            ...
            # Optimization/refactoring
                ...
                perf_impact = (
                    ...
                    if deletions > 50
                    else "improving response time by ~10-15ms"
                )
...
{files_str}"""
        ...
        # Utility/library changes
            ...
            if deletions > additions and any(
                "loop" in f or "array" in f for f in util_files
            ):
...
{files_str}"""
        ...
        # Generic technical summary
            ...
            complexity = (
                "high" if additions > 100 else "moderate" if additions > 50 else "low"
            )
            risk = (
                "high"
                if len(files) > 10 and deletions > 100
                else "medium" if len(files) > 5 else "low"
            )
...
- Primary changes in: {', '.join(list({f.split('/')[0] for f in files if '/' in f})[:3]) if files else 'root'}
