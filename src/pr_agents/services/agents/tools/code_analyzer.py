...
class CodeAnalyzerTool(BaseTool):
    ...
    async def run(self, file_diffs: List[Dict[str, Any]], 
    ...
    def _analyze_complexity(self, file_diffs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_changes = sum(d.get("additions", 0) + d.get("deletions", 0) for d in file_diffs)
    ...
    def _assess_impact(self, file_diffs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for diff in file_diffs:
            ...
            if any(critical in filename for critical in ["core", "auth", "security", "config"]):
                critical_files += 1
                core_changes = True
