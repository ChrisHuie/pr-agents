...
class PatternDetectorTool(BaseTool):
    ...
    def _detect_prebid_patterns(self, file_list: List[str]) -> Dict[str, Any]:
        ...
        for filepath in file_list:
            ...
            # Core changes
            if any(core in filepath.lower() for core in ["src/", "core/", "auction", "bidding"]):
        ...
        if patterns["adapter_changes"]["detected"]:
            ...
            insights.append(f"Modifies {len(adapters)} adapter(s): {', '.join(adapters)}")
        ...
        if patterns["module_changes"]["detected"]:
            ...
            insights.append(f"Updates {len(modules)} module(s): {', '.join(modules[:3])}")
        
        if not patterns["test_coverage"]["detected"] and (
            patterns["adapter_changes"]["detected"] or patterns["core_changes"]["detected"]
        ):
    ...
    def _assess_prebid_risk(self, patterns: Dict[str, Any]) -> List[str]:
        ...
        if patterns["adapter_changes"]["detected"] and not patterns["test_coverage"]["detected"]:
    ...
    def _detect_generic_patterns(self, file_list: List[str]) -> Dict[str, Any]:
        ...
        for filepath in file_list:
            ...
            if any(config in filepath.lower() for config in ["config", ".json", ".yaml", ".yml"]):
        ...
        return {
            "patterns": patterns,
            "primary_language": list(patterns["language_types"])[0] if patterns["language_types"] else "unknown",
