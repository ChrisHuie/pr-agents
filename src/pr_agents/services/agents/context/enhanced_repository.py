...
class EnhancedRepositoryContextProvider:
    ...
    def __init__(self, knowledge_base_path: Path | None = None):
        ...
        if knowledge_base_path is None:
            ...
            knowledge_base_path = Path(__file__).parent.parent.parent.parent.parent.parent / "config" / "repository-knowledge"
    ...
    def _load_knowledge_base(self, repo_type: str) -> Optional[dict]:
    ...
    def get_context(self, repo_url: str, files_changed: list[str]) -> dict[str, Any]:
        ...
        if not base_knowledge:
            logger.debug(f"No knowledge base found for repo type: {repo_type}, using generic context")
        ...
        context["relevant_examples"] = self._get_relevant_examples(base_knowledge, files_changed)
        
        # Add patterns and conventions
        context["patterns"] = self._get_relevant_patterns(base_knowledge, files_changed)
        
        # Add quality checklist
        context["quality_checklist"] = self._get_quality_checklist(base_knowledge, files_changed)
        
        # Add common issues
        context["common_issues"] = self._get_common_issues(base_knowledge, files_changed)
        
        # Add file-specific guidance
        context["file_guidance"] = self._get_file_guidance(base_knowledge, files_changed)
    ...
    def _get_relevant_examples(self, knowledge: dict, files_changed: list[str]) -> list[dict]:
        ...
        for file in files_changed:
            ...
            if re.search(r'BidAdapter\.(js|java|go|swift|kt)$', file):
                if 'bid_adapter' in code_examples:
                    examples.append({
                        ...
                        'code': code_examples['bid_adapter'].get('complete_example', {}).get('code', '')
                    })
                    break
            ...
            # Test files
            elif re.search(r'_spec\.(js|java|go|swift|kt)$|Test\.(java|swift|kt)$', file):
                test_key = 'test_patterns' if 'test_patterns' in code_examples else 'adapter_test'
                if test_key in code_examples:
                    examples.append({
                        'type': 'test',
                        'description': 'Test structure reference',
                        'code': code_examples[test_key].get('adapter_test', {}).get('code', '') if test_key == 'test_patterns' else code_examples[test_key].get('code', '')
                    })
        ...
        for file in files_changed:
            if 'adapter' in file.lower():
                if 'adapter_structure' in patterns:
                    relevant_patterns['adapter_structure'] = patterns['adapter_structure']
    ...
    def _get_quality_checklist(self, knowledge: dict, files_changed: list[str]) -> list[str]:
    ...
    def _get_common_issues(self, knowledge: dict, files_changed: list[str]) -> list[dict]:
    ...
    def _get_file_guidance(self, knowledge: dict, files_changed: list[str]) -> dict:
        ...
        for file in files_changed:
            if re.search(r'BidAdapter\.(js|java|go|swift|kt)$', file):
                guidance['adapter'] = {
                    'required_methods': knowledge.get('patterns', {}).get('adapter_structure', {}).get('required_methods', []),
                    'optional_methods': knowledge.get('patterns', {}).get('adapter_structure', {}).get('optional_methods', []),
                    'common_imports': knowledge.get('patterns', {}).get('imports', {}).get('required', [])
                }
