...
from src.pr_agents.services.ai.feedback import FeedbackEntry, FeedbackStore
...
class TestAIServiceIntegration:
    ...
    async def test_cost_optimized_generation(self):
        ...
        mock_provider.generate = AsyncMock(
            return_value=Mock(content="Test summary")
        )
        ...
        # Generate summaries
        with patch.object(service.cost_optimizer, 'select_optimal_provider', 
    ...
    async def test_streaming_generation(self):
        ...
        with patch('src.pr_agents.services.ai.service.AIConfig') as mock_config, \
             patch('src.pr_agents.services.ai.service.PromptBuilder') as mock_prompt_builder:
    ...
    def test_feedback_integration(self, tmp_path):
        ...
        with patch.object(FeedbackStore, '__init__', 
                         lambda self: setattr(self, 'storage_path', tmp_path / "feedback.json") or
                         setattr(self, 'feedback_entries', [])):
            service = AIService(
                provider=mock_provider,
