...
class TestConfigurationEdgeCases:
    ...
    def test_circular_inheritance(self, tmp_path):
        ...
        config_a = repos_dir / "a.json"
        config_a.write_text(
            json.dumps(
                {"extends": "b.json", "repo_name": "test/a", "repo_type": "test"}
            )
        )

        # Create config B that extends A
        config_b = repos_dir / "b.json"
        config_b.write_text(
            json.dumps(
                {"extends": "a.json", "repo_name": "test/b", "repo_type": "test"}
            )
        )

        # Also create a valid config without circular dependency
        config_c = repos_dir / "c.json"
        config_c.write_text(json.dumps({"repo_name": "test/c", "repo_type": "test"}))

        loader = ConfigurationLoader(str(tmp_path))

        # The main assertion is that this doesn't cause infinite recursion
        # The loader should handle circular inheritance gracefully
        config = loader.load_config()

        # The loader may skip the circular configs or load them partially
        # At minimum, the non-circular config should load
        from src.pr_agents.config.models import RepositoryConfig

        assert isinstance(config, RepositoryConfig)
        assert "test/c" in config.repositories
    ...
    def test_invalid_pattern_types(self):
        ...
        with pytest.raises(InvalidPatternError):
            pattern = ModulePattern(
                pattern="suffix_without_star", pattern_type="suffix"
            )
        ...
        with pytest.raises(InvalidPatternError):
            pattern = ModulePattern(
                pattern="prefix_without_star", pattern_type="prefix"
            )
            matcher.match_pattern("test.js", pattern)
    ...
    def test_concurrent_config_modifications(self, tmp_path):
        ...
        def modify_config():
            for i in range(5):
                config_file.write_text(
                    json.dumps({"repo_name": "test/repo", "repo_type": f"test-{i}"})
                )
                time.sleep(0.1)
    ...
    def test_watcher_rapid_changes(self, tmp_path):
        ...
        with ConfigurationWatcher(str(tmp_path), callback=on_change):
            # Rapid modifications
            for i in range(10):
                config_file.write_text(
                    json.dumps({"repo_name": "test", "repo_type": f"test-{i}"})
                )
