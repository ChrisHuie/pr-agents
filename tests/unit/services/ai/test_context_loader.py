...
class TestAgentContextLoader:
    ...
    def temp_project_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (project_root / "CLAUDE.md").write_text("# Project Context\nTest project context")
            (project_root / "claude.md").write_text("# Project Agent Context\nClaude-specific")
            
            # Create agent context directory structure
            agent_dir = project_root / "config" / "agent-context" / "repositories" / "prebid-js"
            agent_dir.mkdir(parents=True)
            
            # Create repository-specific context files
            (agent_dir / "claude.md").write_text("# Prebid.js Claude Context\nRepository-specific Claude context")
            (agent_dir / "gemini.md").write_text("# Prebid.js Gemini Context\nRepository-specific Gemini context")
            (agent_dir / "agents.md").write_text("# Prebid.js ADK Context\nRepository-specific ADK context")
    ...
    def test_load_context_for_pr_claude(self, temp_project_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_project_dir):
            ...
            assert (
                ("Project Context (CLAUDE.md)" in context and "Test project context" in context) or
                ("Prebid.js Repository Context" in context)
            )
    ...
    def test_load_context_for_pr_gemini(self, temp_project_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_project_dir):
            ...
            assert context  # Not empty
            assert (
                "Prebid.js Gemini Context" in context or
                "Prebid.js Repository Context" in context
            )
    ...
    def test_load_context_for_pr_adk(self, temp_project_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_project_dir):
            ...
            assert context  # Not empty
            assert (
                "Prebid.js ADK Context" in context or
                "Prebid.js Repository Context" in context
            )
    ...
    def test_create_repository_context_structure(self, temp_project_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_project_dir):
            ...
            repo_dir = loader.create_repository_context_structure("prebid/prebid-server")
    ...
    def test_read_file_error_handling(self, temp_project_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_project_dir):
            ...
            with patch("src.pr_agents.services.ai.context_loader.logger") as mock_logger:
