...
class TestPromptBuilder:
    ...
    def test_build_prompt_executive(self, builder, code_changes, repo_context, pr_metadata):
        """Test building executive prompt."""
        prompt = builder.build_prompt("executive", code_changes, repo_context, pr_metadata)
    ...
    def test_build_prompt_product(self, builder, code_changes, repo_context, pr_metadata):
        """Test building product prompt."""
        prompt = builder.build_prompt("product", code_changes, repo_context, pr_metadata)
    ...
    def test_build_prompt_developer(self, builder, code_changes, repo_context, pr_metadata):
        """Test building developer prompt."""
        prompt = builder.build_prompt("developer", code_changes, repo_context, pr_metadata)
        
        assert "Technical Summary:" in prompt
        assert "Test files: 1" in prompt
        assert "modules/: 1 files, 100 total changes" in prompt

    def test_build_prompt_with_agent_context(self, builder, code_changes, repo_context, pr_metadata):
    ...
    def test_build_prompt_without_agent_context(self, builder, code_changes, repo_context, pr_metadata):
    ...
    def test_build_prompt_unknown_persona(self, builder, code_changes, repo_context, pr_metadata):
    ...
    def test_extract_template_variables(self, builder, code_changes, repo_context, pr_metadata):
    ...
    def test_empty_code_changes(self, builder, repo_context, pr_metadata):
        ...
        prompt = builder.build_prompt("executive", empty_changes, repo_context, pr_metadata)
        
        assert "0 files modified" in prompt
        assert "0 additions" in prompt

    @pytest.mark.parametrize("persona", ["executive", "product", "developer"])
    def test_all_personas_with_agent_context(self, builder, code_changes, repo_context, pr_metadata, persona):
